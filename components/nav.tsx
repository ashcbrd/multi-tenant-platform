...
import { getSiteFromInfoId } from "@/lib/actions";
...
export default function Nav({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (segments[0] === "info" && id) {
      getSiteFromInfoId(id).then((id) => {
        setSiteId(id);
      });
    }
  }, [segments, id]);
  ...
  const tabs = useMemo(() => {
    ...
    } else if (segments[0] === "info" && id) {
      return [
        {
          name: "Back to All Venues",
    ...
    <>
      <button
        className={`fixed z-20 ${
          ...
          segments[0] === "info" && segments.length === 2 && !showSidebar
            ? "left-5 top-5"
            : "right-5 top-7"
        } sm:hidden`}
